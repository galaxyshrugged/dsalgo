...
public class LongestPathInMaze
{
 public static void main(String[] args)
 {
  ...
  makeRandomMaze(maze);
  printMaze(maze);
  List path = findLongestPath(maze);
  if (path == null)
  {
 ...
 private static List findLongestPath(boolean[][] maze)
 {
  Cell start = new Cell(0, 0);
  Cell end = new Cell(maze.length - 1, maze[0].length - 1);
  List path = findLongestPath(maze, start, end);
  return path;
 }

 private static List findLongestPath(boolean[][] maze, Cell start,
   Cell end)
 {
  List result = null;
  ...
  {
   List path = findLongestPath(maze, nextCell, end);
